// Simplified iOS 26 (Liquid Glass) device frame — ported from ios-frame.jsx.
// No assets, no deps. Used as a presentation shell around the app.
...
export function IOSStatusBar({ dark = false, time = '9:41' }: { dark?: boolean; time?: string }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 154, alignItems: 'center', justifyContent: 'center', padding: '21px 24px 19px', boxSizing: 'border-box', position: 'relative', zIndex: 20, width: '100%', height: '31px' }}>
...
export function IOSListRow({ title, detail, icon, chevron = true, isLast = false, dark = false }: { title: ReactNode; detail?: ReactNode; icon?: string; chevron?: boolean; isLast?: boolean; dark?: boolean }) {
  const text = dark ? '#fff' : '#000';
...
// ── Device frame ──────────────────────────────────────────────
export function IOSDevice({ children, width = 402, height = 874, dark = false, title, keyboard = false }: { children?: ReactNode; width?: number; height?: number; dark?: boolean; title?: string; keyboard?: boolean }) {
  return (
